...
Turns one blog idea your organisation already holds into a finished post with its featured image and the LinkedIn post that announces it. You give a brief, pick one idea no draft has used yet, wait while the post and its picture are made, review the post while the run waits, and approve the LinkedIn post.
...
**Review:** the run pauses three times — to pick an idea, to review what the run made, and to approve the LinkedIn post. The middle pause names the revisions the run minted, so the review opens on exactly those, one per thing, and the run waits until the last is decided. This flow owns the pause, not the decision.
