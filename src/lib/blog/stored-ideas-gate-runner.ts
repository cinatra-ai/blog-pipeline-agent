/**
 * THE PIPELINE'S STORED-IDEAS GATE, the part that reads and writes.
 *
 ...
 *                          pipeline's DECLARED dependency, read each candidate's
 *                          text (one read per unused idea), subtract what the
 *                          relation table holds, and emit the rest as the message
 *                          the gate renderer already reads.
...
} from "./stored-ideas-gate.ts";

/** How many stored ideas one gate reads content for. The listing is paged at a
 *  hundred per page; a person choosing from more than this is choosing from a
 *  list nobody reads, and each entry costs a content read. */
